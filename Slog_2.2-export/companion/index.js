...
function printAccelLog(data) {  
  ...
  while (readPos < dataSize) {    
    ...
    for (let i = 0; i < accelConfig.batch; i++) {
      ...
      content += `A, ${accelRecordTimeView[i]}, ${accelRecordXView[i].toFixed(9)}, ${accelRecordYView[i].toFixed(9)}, ${accelRecordZView[i].toFixed(9)}\n`;
    }
  }
...
function printGyroLog(data) {
  ...
  while (readPos < dataSize) {
    ...
    for (let i = 0; i < gyroConfig.batch; i++) {
      ...
      content += `G, ${gyroRecordTimeView[i]}, ${gyroRecordXView[i].toFixed(9)}, ${gyroRecordYView[i].toFixed(9)}, ${gyroRecordZView[i].toFixed(9)}\n`;
    }
  }
